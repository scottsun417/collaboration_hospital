...
```{r}
library(caret)
index <- createDataPartition(os_data$os, p=.8, list=FALSE, times=1)
```

# Create test and train data frames
```{r}
train_df <- os_data[index,]
test_df <- os_data[-index,]
```
...
model_step2 = step(null_model, 
                  scope = list(lower=null_model, upper=full_model),
                  direction = "forward", k= log(nrow(os_data)), trace=0) 
```
```{r}
#summary model
summary(model_step2)
...
model_step4 = step(full_model,
                  direction = "backward", k= log(nrow(os_data)), trace=0) 
```
...
model_step6 = step(null_model, 
                  scope = list(lower=null_model, upper=full_model),
                  direction = "both", k= log(nrow(os_data)), trace=0)
```
