...
cal1 <- calibrate(fit1_lg, method='boot', B=1000)
...
cal2 <- calibrate(fit2_lg, method='boot', B=1000)
...
```{r}
fit_A <- glm(N~age+t+grade+tlocate+kind+cea+pin, data = df, family = binomial(link="logit"),x=TRUE)
fit_B <- glm(N~t+grade+kind+cea+pin, data = df, family = binomial(link="logit"),x=TRUE)
